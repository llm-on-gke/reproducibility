...
gcloud compute tpus tpu-vm scp config.json fsdp_config.json train.sh host.sh env.sh install_packages.sh test.sh $TPU_NAME:~ --worker=all --project $PROJECT --zone=$ZONE
